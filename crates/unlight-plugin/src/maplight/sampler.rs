...
fn evaluate_beam(
    rpos_raw: Position,
    lpos_unrot: Position,
    unrot_axes: [Direction; 3],
    dir_dist: f32,
    power_f: f32,
    beam_focus: f32,
) -> (f32, f32) {
    let mut rpos = Position {
        x: rpos_raw.x * unrot_axes[0].dx
            + rpos_raw.y * unrot_axes[1].dx
            + rpos_raw.z * unrot_axes[2].dx,
        y: rpos_raw.x * unrot_axes[0].dy
            + rpos_raw.y * unrot_axes[1].dy
            + rpos_raw.z * unrot_axes[2].dy,
        z: rpos_raw.x * unrot_axes[0].dz
            + rpos_raw.y * unrot_axes[1].dz
            + rpos_raw.z * unrot_axes[2].dz,
    ...
    if spot_rpos.x >= 0.0 {
        spot_rpos.x = fastapprox::faster::pow(spot_rpos.x, 1.0 / beam_focus.clamp(1.0, 1.3));
        spot_rpos.y /= spot_rpos.x * (beam_focus - 1.0).clamp(0.0, 10.0) / 30.0 + MIN_SPREAD;
    } else {
        spot_rpos.x =
            -fastapprox::faster::pow(-spot_rpos.x, (beam_focus / 5.0 + 1.0).clamp(1.0, 4.0));
        spot_rpos.y *= -spot_rpos.x * (beam_focus - 1.0).clamp(0.0, 10.0) / 30.0 + 1.0;
    ...
    let dist =
        (emitter_pos.distance(&spot_rpos) + 0.5).powf((dir_dist / 200.0).clamp(0.5, 1.0).recip());
    let spot_fl = power_f / (dist + 0.5);

    let beam_len = dir_dist / 30.0;
    let x_player_rel = rpos.x + beam_len;
    let mut cone_fl = 0.0;
    if x_player_rel > 0.0 && rpos.x < 0.0 {
        let progress = (x_player_rel / beam_len).clamp(0.0, 1.0);
        let range_factor = ((dir_dist - 150.0) / 250.0).clamp(0.0, 1.0);
        let angle_multiplier = 1.0 - (range_factor * 0.6);
        let cone_width = (progress * 15.0 + 5.0) * angle_multiplier;

        let dz = rpos.z - lpos_unrot.z;
        let lateral_dist = (rpos.y * rpos.y + dz * dz).sqrt();
        let angular_falloff = (1.0 - (lateral_dist / cone_width)).clamp(0.0, 1.0).powi(2);
        let intensity_mod = progress.powi(2) * (1.0 - progress) * 6.75;
        cone_fl = power_f * 0.20 * intensity_mod * angular_falloff;
    }
    ...
    pub(crate) fn fpos_gamma_color(
        ...
        for flash in self.flashlights.iter() {
            ...
            let fl = if is_light_sensitive && d2 < 4.0 {
                ...
                flash.power / (d2.sqrt() + 5.0) * 2.0
            } else {
                let (spot, cone) = evaluate_beam(
                    rpos_raw,
                    flash.lpos_unrot,
                    flash.unrot_axes,
                    flash.dir.distance(),
                ...
                let direct_fl = spot + cone;

                let bounce_fl = if flash.bounce_valid {
                    let (b_spot, b_cone) = evaluate_beam(
                        rpos_raw,
                    ...
                    // Specular bounce tweak: bright cone, dim spotlight
                    b_spot * 0.5 + b_cone * 6.0
                } else {
                    0.0
                };

                (direct_fl + bounce_fl) * flvis.clamp(0.0001, 1.0)
            };

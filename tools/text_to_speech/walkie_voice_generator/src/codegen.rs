...
pub fn generate_rust_code(
    ...
    for (ron_filename_str, entries_for_ron_file) in ron_to_lines {
        ...
        for concept_id in concept_to_lines.keys() {
        ...
        let mut sorted_concepts: Vec<_> = concept_to_lines.into_iter().collect();
        sorted_concepts.sort_by_key(|(k, _)| k.clone());

        for (concept_id, lines_for_concept) in sorted_concepts {
            rs_file_content.push_str(&format!(
                "            Self::{} => vec![\n",
                concept_id
            ));
            let mut sorted_lines = lines_for_concept;
            sorted_lines.sort_by_key(|l| l.line_index);
            ...
            for line_data in sorted_lines {
                ...
                for (idx, tag) in line_data.tags.iter().enumerate() {
                    if idx > 0 {
                        rs_file_content.push_str(", ");
                    }
    ...
    let mut mod_file = File::create(&mod_rs_path)
        .map_err(|e| anyhow::anyhow!("Failed to create mod.rs file {:?}: {}", mod_rs_path, e))?;
    mod_file
        .write_all(mod_rs_content.as_bytes())
        .map_err(|e| anyhow::anyhow!("Failed to write to mod.rs file {:?}: {}", mod_rs_path, e))?;

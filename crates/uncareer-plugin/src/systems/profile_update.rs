...
/// Reacts to `CareerRewardCalculatedEvent` and persists the outcome to `PlayerProfileData`.
/// Uses `Option<ResMut<...>>` so the system is safe on dedicated servers where no local
/// profile exists.
pub(crate) fn apply_career_reward_to_profile(
    mut ev_reward: MessageReader<CareerRewardCalculatedEvent>,
    player_profile: Option<ResMut<Persistent<PlayerProfileData>>>,
    mut sd: Option<ResMut<SummaryData>>,
) {
    let Some(mut player_profile) = player_profile else {
        return;
    };
...
/// Reacts to `CareerDeathRecordedEvent` and persists the outcome to `PlayerProfileData`.
/// Uses `Option<ResMut<...>>` so the system is safe on dedicated servers where no local
/// profile exists.

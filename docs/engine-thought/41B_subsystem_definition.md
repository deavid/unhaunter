...
In previous design phases, we focused on "Components" (in the ECS sense). This was too granular. It led to listing
individual structs like `ThermalMass` or `Switch` rather than seeing the bigger picture.

We need a term for the **"Big Chunks"** of the engine. The architectural pillars. The things that might become their own
crates.
...
If the answer is "No, it's just a struct," it's a Component. If the answer is "No, it's just a function," it's a System.

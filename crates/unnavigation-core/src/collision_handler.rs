...
impl CollisionHandler<'_> {
    ...
    pub fn spectator_delta(&self, pos: &Position) -> Vec3 {
        let mut delta = Vec3::ZERO;
        ...
        // First check collisions on the current floor
        delta += self.check_spectator_floor_collisions(&bpos, pos);
        ...
        if between_floors {
            ...
            // Check collisions on the adjacent floor
            delta += self.check_spectator_floor_collisions(&adjacent_bpos, pos);
        }

        delta
    }

    // Helper method to check collisions on a specific floor for spectators
    fn check_spectator_floor_collisions(&self, bpos: &BoardPosition, pos: &Position) -> Vec3 {
        let mut delta = Vec3::ZERO;

        for npos in bpos.iter_xy_neighbors_nosize(1) {
            let cf = self.bcf.0.get(npos.ndidx()).copied().unwrap_or_default();

            // For spectators, we check !ghost_free (can't move into void/invalid space)
            // instead of !player_free (can't move into walls/objects)
            if !cf.ghost_free && Self::ENABLE_COLLISION {
                let dpos = npos.to_position().to_vec3() - pos.to_vec3();

                // We use the same collision calculation logic as normal players
                // This treats the "void" tile as a solid block/pillar
                let mut dapos = dpos.abs();
                dapos.z = 0.0;
                dapos.x -= Self::PILLAR_SZ;
                dapos.y -= Self::PILLAR_SZ;

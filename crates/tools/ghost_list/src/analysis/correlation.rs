...
pub fn handle_correlation_command(evidence1_str: &str, evidence2_str: Option<&str>) {
    ...
    let evidence1 = match all::<Evidence>().find(|e| e.name().eq_ignore_ascii_case(evidence1_str)) {
        ...
        None => {
            eprintln!(
                "Error: Could not parse primary evidence '{}'.",
                evidence1_str
            );
            return;
        }
    ...
    if let Some(e2_str) = evidence2_str {
        ...
        println!(
            ...
            evidence1.name(),
            count_e1,
            (count_e1 as f32 / total_ghosts as f32) * 100.0
        );
        println!(
            "Ghosts with '{}': {} ({:.1}%)",
            evidence2.name(),
            count_e2,
            (count_e2 as f32 / total_ghosts as f32) * 100.0
        );
        println!("--------------------------------------------------");
        println!(
            "Ghosts with BOTH '{}' and '{}': {} ({:.1}%)",
            evidence1.name(),
            evidence2.name(),
            count_both,
            (count_both as f32 / total_ghosts as f32) * 100.0
        );
        println!(
            "Ghosts with '{}' but NOT '{}': {} ({:.1}%)",
            evidence1.name(),
            evidence2.name(),
            count_e1_not_e2,
            (count_e1_not_e2 as f32 / total_ghosts as f32) * 100.0
        );
        println!(
            "Ghosts with '{}' but NOT '{}': {} ({:.1}%)",
            evidence2.name(),
            evidence1.name(),
            count_e2_not_e1,
            (count_e2_not_e1 as f32 / total_ghosts as f32) * 100.0
        );
        println!(
            "Ghosts with NEITHER '{}' NOR '{}': {} ({:.1}%)",
            evidence1.name(),
            evidence2.name(),
            count_neither,
            (count_neither as f32 / total_ghosts as f32) * 100.0
        );
        ...
        if count_e2 > 0 {
            println!(
                ...
                evidence1.name(),
                evidence2.name(),
                count_both as f32 / count_e2 as f32
            );
        }
        ...
        if count_e1 > 0 {
            println!(
                ...
                evidence2.name(),
                evidence1.name(),
                count_both as f32 / count_e1 as f32
            );
        }
    } else {
        ...
        for ghost in &all_ghosts {
            if ghost.evidences().contains(&evidence1) {
                ...
                for other_evidence_type in all::<Evidence>() {
                    if other_evidence_type == evidence1 {
                        continue;
                    }
        ...
        println!(
            "\nTotal ghosts with '{}': {} out of {}",
            evidence1.name(),
            count_e1_total,
            total_ghosts
        );
        println!(
            "\nCorrelation with other evidences (P(Other | {})):",
            evidence1.name()
        );
        ...
        for (other_evidence, (count_both_with_e1, count_other_total)) in sorted_correlations {
            let p_other_given_e1 = if count_e1_total > 0 {
                count_both_with_e1 as f32 / count_e1_total as f32
            } else {
                0.0
            };
            let p_e1_given_other = if count_other_total > 0 {
                count_both_with_e1 as f32 / count_other_total as f32
            } else {
                0.0
            };

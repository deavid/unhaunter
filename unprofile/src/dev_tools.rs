...
pub fn snapshot_schema_system(player_profile: Res<Persistent<PlayerProfileData>>) {
    if let Some(fixture_dir) = get_fixture_directory() {
        ...
        match to_string_pretty(player_profile.get(), pretty_config) {
            Ok(serialized) => {
                ...
                if path.extension().and_then(|ext| ext.to_str()) == Some("ron") {
                    match fs::read_to_string(&path) {
                        Ok(content) => match from_str::<PlayerProfileData>(&content) {
                            ...
                            Err(e) => {
                                panic!("Schema validation failed for fixture {:?}: {:?}", path, e);
                            }
                        },
                        Err(e) => {

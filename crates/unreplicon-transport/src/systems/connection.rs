...
pub(super) fn app_setup(app: &mut App) {
    let condition = (resource_exists::<RuntimeInstallationId>.or(resource_exists::<AuthorityRole>))
        .and(not(resource_exists::<NetcodeClientTransport>))
        .and(|config: Res<TransportConfig>| !matches!(*config, TransportConfig::Offline));

    #[cfg(not(target_arch = "wasm32"))]
    let condition = condition.and(not(resource_exists::<NetcodeServerTransport>));

    app.add_systems(Update, startup_transport_system.run_if(condition));
...
fn startup_transport_system(
    ...
    match &*transport_config {
        ...
        TransportConfig::PeerHost {
            ...
            #[cfg(target_arch = "wasm32")]
            {
                let _ = (port, bind_addresses);
                panic!("PeerHost is not supported on WASM");
            }
            #[cfg(not(target_arch = "wasm32"))]
            {
                let port = *port;
                ...
                let (wt_config, cert_hash) =
                    renet2_netcode::WebTransportServerConfig::new_selfsigned(wt_addr, MAX_CLIENTS)
                        .unwrap();
                let wt_socket =
                    renet2_netcode::WebTransportServer::new(wt_config, tokio_handle.clone())
                        .unwrap();

                let ws_addr = SocketAddr::from(([0, 0, 0, 0], ws_port));
                let ws_config = renet2_netcode::WebSocketServerConfig::new(ws_addr, MAX_CLIENTS);
                let ws_socket =
                    renet2_netcode::WebSocketServer::new(ws_config, tokio_handle).unwrap();
